...
class Program
{
    ...
    {
        Console.WriteLine("Fetching latest GHelper stable release...");
        ...
        try
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                string? downloadUrl = releaseInfo["assets"]?.FirstOrDefault(a => a["browser_download_url"]?.ToString()?.EndsWith(".zip", StringComparison.OrdinalIgnoreCase) ?? false)?["browser_download_url"]?.ToString();

                string version = releaseInfo["tag_name"]?.ToString() ?? "unknown";

                // Output the information to the console
                if (downloadUrl != null)
                {
                    Console.WriteLine("Stable release found.");
